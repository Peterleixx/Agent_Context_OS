...
def build_site_payload(persona_vault_path: Path, site_title: str | None) -> dict[str, str]:
    ...
    return {
        ...
        "HERO_POINTS": render_list(coerce_string_list(render_profile.get("public_summary", [])), tone="dark"),
        "PROFILE_FACETS": render_profile_facets(list(render_profile.get("profile_facets", []))),
        "KEYWORD_CHIPS": render_keyword_chips(coerce_string_list(render_profile.get("keyword_chips", []))),
